...
def _advance_intro(session):
    """Walk through the human greeting turns (greet -> identify+ask-time -> ask-code)
    until the call is actually asking for the verification code. Any reply advances
    the deterministic intro."""
    for _ in range(5):
        if session.state.value != "greeting":
            break
        orchestrator.customer_says(session, "yes, speaking")
...
# --- new coverage: the Investigation Agent (multi-signal reasoning on ambiguous cases) ---
# The value prop is combination-of-signals: one suspicious signal is explainable,
# two-or-more together is fraud. These lock that reasoning in (MOCK mode = the
# deterministic heuristic, so no network).

def _inv_case(**kw):
    base = dict(customer_name="X", merchant="M", merchant_category="Retail", amount=100.0,
                known_devices=["iPhone 14"], current_device="iPhone 14",
                known_locations=["London"], current_location="London", velocity_minutes=200)
    base.update(kw)
    return base


def test_investigation_two_signals_is_fraud():
    case = _inv_case(current_device="unknown Android device", current_location="Lagos")  # device + location
    r = investigation_agent.investigate(case, p95_gbp=500)
    assert r.predicted_fraud and r.recommendation == "block_and_escalate" and r.signals_flagged >= 2


def test_investigation_single_signal_is_borderline_not_fraud():
    case = _inv_case(current_location="Lagos")  # foreign location only; device recognised
    r = investigation_agent.investigate(case, p95_gbp=500)
    assert not r.predicted_fraud and r.recommendation == "human_review" and r.signals_flagged == 1


def test_investigation_clean_case_clears():
    r = investigation_agent.investigate(_inv_case(), p95_gbp=500)
    assert not r.predicted_fraud and r.recommendation == "clear" and r.signals_flagged == 0


def test_investigation_catches_device_velocity_fraud_scorer_would_miss():
    """A new device + card-testing velocity at HOME with a normal amount: the
    amount+geo scorer sees nothing (home city, small spend) and clears it — the
    investigation agent catches it on device+velocity."""
    case = _inv_case(current_device="unknown Android device", velocity_minutes=6)
    r = investigation_agent.investigate(case, p95_gbp=500)
    assert r.predicted_fraud and r.signals_flagged >= 2
...
def test_metrics_record_one_sample_per_llm_call_tagged_by_stage(tmp_path, monkeypatch):
    monkeypatch.setattr(metrics, "METRICS_DIR", tmp_path / "metrics")  # isolate from real logs/metrics/
    metrics._RING.clear()
    session = _verified_session("txn_gadget_02")
...
def test_metrics_fast_path_records_no_dialog_llm_sample(tmp_path, monkeypatch):
