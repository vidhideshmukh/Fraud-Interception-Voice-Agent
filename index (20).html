...
def build_database(json_path: Path = JSON_PATH, db_path: Path = DB_PATH) -> tuple[int, int]:
    ...
    for rows in by_customer.values():
        ...
        # Baseline is computed from the customer's CLEAN, in-pattern history only:
        # not fraud (would inflate the ceiling), and not AMBIGUOUS rows either —
        # an ambiguous-legit row (a moderate spike, a far-UK trip) is a borderline
        # event by construction, so folding it into "normal" would blur the very
        # baseline the ambiguous cases are meant to sit just outside of. Fall back
        # gracefully if a customer somehow has no clean rows.
        baseline = ([r for r in rows if not r["isFraud"] and r.get("scenario") != "ambiguous"]
                    or [r for r in rows if not r["isFraud"]] or rows)
        amounts = [float(r["amount"]) for r in baseline]
