...
def _build_record(cust: dict, is_fraud: bool, rng: random.Random) -> dict:
    ...
    return {
        ...
        "isFraud": is_fraud,
        "scenario": "clear",          # clear-cut: clean, or one baseline decisively broken
        "ambiguous_pattern": "",      # (only set on ambiguous rows)
    }


# ---------------------------------------------------------------------------
# AMBIGUOUS / conflicting-signal patterns — the "messy middle" a fixed threshold
# cannot resolve but a human analyst (or an investigation agent) reasons through.
# Each mixes ONE suspicious signal with a REASSURING one, so amount+geography
# alone (what the current scorer uses) can't decide. The hidden ground-truth
# label is carried by a SUBTLE secondary tell on the dimensions the score
# ignores: fraud rows get a second break (a new device, or a card-testing
# velocity), legit rows don't. So the answer exists in the evidence — just not in
# the two dimensions the blunt score looks at. That gap is exactly what makes
# these worth an INVESTIGATION step (checking device/velocity) instead of a rule.
# ---------------------------------------------------------------------------
AMBIGUOUS_PATTERNS = ["foreign_but_traveller", "moderate_spike", "new_device_at_home",
                      "fast_velocity_small", "far_uk_trip"]


def _unknown_device(cust: dict, rng: random.Random) -> str:
    return rng.choice([d for d in UNKNOWN_DEVICES if d not in cust["devices"]])


def _rebase_times(rec: dict, velocity_minutes: int) -> None:
    """Reset current_transaction_time to previous + velocity, keeping the pair coherent."""
    prev = datetime.fromisoformat(rec["previous_transaction_time"].replace("Z", "+00:00"))
    rec["velocity_minutes"] = velocity_minutes
    rec["current_transaction_time"] = _iso(prev + timedelta(minutes=velocity_minutes))


def _build_ambiguous_record(cust: dict, is_fraud: bool, rng: random.Random) -> dict:
    """A conflicting-signal transaction. Starts from a fully in-pattern legit row,
    applies one ambiguous pattern (suspicious + reassuring), and — if is_fraud —
    adds the subtle secondary tell on a dimension the blunt scorer ignores."""
    rec = _build_record(cust, is_fraud=False, rng=rng)   # start clean & in-pattern
    pattern = rng.choice(AMBIGUOUS_PATTERNS)

    if pattern == "foreign_but_traveller":
        # Suspicious: a charge abroad. Reassuring: amount in-pattern, believable travel gap.
        rec["current_location"] = rng.choice(FOREIGN_CITIES)
        if is_fraud:                     # tell: a real traveller uses their own phone
            rec["current_device"] = _unknown_device(cust, rng)
    elif pattern == "moderate_spike":
        # Suspicious: amount a bit over their ceiling (1.5-2.4x). Reassuring: home + known device.
        rec["amount"] = round(cust["high_gbp"] * rng.uniform(1.5, 2.4), 2)
        if is_fraud:
            rec["current_device"] = _unknown_device(cust, rng)
    elif pattern == "new_device_at_home":
        # Suspicious: a device never seen. Reassuring: home city + normal amount (a phone upgrade?).
        rec["current_device"] = _unknown_device(cust, rng)
        if is_fraud:                     # tell: card-testing pace, not settling into a new phone
            _rebase_times(rec, rng.randint(4, 20))
    elif pattern == "fast_velocity_small":
        # Suspicious: a rapid second charge. Reassuring: small amount, usual place.
        _rebase_times(rec, rng.randint(6, 30))
        rec["amount"] = round(cust["avg_gbp"] * rng.uniform(0.3, 0.8), 2)
        if is_fraud:
            rec["current_device"] = _unknown_device(cust, rng)
    elif pattern == "far_uk_trip":
        # Suspicious: a far UK city they don't usually use. Reassuring: known device, moderate amount.
        rec["current_location"] = rng.choice(FAR_UK_CITIES)
        rec["amount"] = round(cust["high_gbp"] * rng.uniform(0.6, 1.1), 2)
        if is_fraud:
            rec["current_device"] = _unknown_device(cust, rng)

    rec["isFraud"] = is_fraud
    rec["scenario"] = "ambiguous"
    rec["ambiguous_pattern"] = pattern
    return rec


def _mix(n: int, fraud_rate: float, ambiguous_rate: float, rng: random.Random) -> list[dict]:
    """Build a shuffled mix of clear-cut and ambiguous records. Ambiguous rows are
    ~50/50 fraud/legit (the messy middle is genuinely balanced); clear rows use
    fraud_rate. Labels stay correct by construction in both classes."""
    n_amb = round(n * ambiguous_rate)
    n_clear = n - n_amb
    n_clear_fraud = round(n_clear * fraud_rate)
    clear = [_build_record(rng.choice(CUSTOMERS), f, rng)
             for f in ([True] * n_clear_fraud + [False] * (n_clear - n_clear_fraud))]
    n_amb_fraud = round(n_amb * 0.5)
    amb = [_build_ambiguous_record(rng.choice(CUSTOMERS), f, rng)
           for f in ([True] * n_amb_fraud + [False] * (n_amb - n_amb_fraud))]
    records = clear + amb
    rng.shuffle(records)
    return records


def generate_python(n: int, fraud_rate: float, ambiguous_rate: float, seed: int) -> list[dict]:
    """Pure-Python engine: n records mixing clear-cut + ambiguous, labels correct
    by construction. Deterministic for a given seed so a run is reproducible."""
    return _mix(n, fraud_rate, ambiguous_rate, random.Random(seed))


def generate_datadesigner(n: int, fraud_rate: float, ambiguous_rate: float, seed: int, model: str) -> list[dict]:
    ...
    b.add_column(dd.SamplerColumnConfig(name="is_ambiguous", sampler_type=dd.SamplerType.BERNOULLI,
                 params=dd.BernoulliSamplerParams(p=ambiguous_rate)))
    ...
    for row in df.itertuples(index=False):
        ...
        # Data Designer sampled whether this row is clear-cut or ambiguous; the
        # signal/label stamping (coherent by construction) is done by our builders.
        # The ambiguous class is forced to ~50/50 fraud/legit (a balanced hard-case
        # set for eval) rather than inheriting the low clear-cut fraud_rate.
        if bool(int(r["is_ambiguous"])):
            rec = _build_ambiguous_record(cust, rng.random() < 0.5, rng)
        else:
            rec = _build_record(cust, is_fraud, rng)
...
def _write(records: list[dict]) -> None:
    ...
    try:
        ...
        # The CSV is only for eyeballing; if it's open in Excel/the editor the JSON
        # (the real artifact) is already written — warn and carry on, don't crash.
        print(f"[warn] could not write {csv_path.name} (is it open in an editor?) — JSON is fine.")

    n_fraud = sum(1 for r in records if r["isFraud"])
    n_amb = sum(1 for r in records if r.get("scenario") == "ambiguous")
    n_amb_fraud = sum(1 for r in records if r.get("scenario") == "ambiguous" and r["isFraud"])
    ...
    print(f"       scenarios: {len(records) - n_amb} clear-cut, {n_amb} ambiguous "
          f"({n_amb_fraud} fraud / {n_amb - n_amb_fraud} legit in the ambiguous class)")
...
def main():
    ...
    p.add_argument("--n", type=int, default=200, help="number of records")
    p.add_argument("--fraud-rate", type=float, default=0.35,
                   help="fraction of the CLEAR-CUT rows that are fraud (ambiguous rows are ~50/50)")
    p.add_argument("--ambiguous-rate", type=float, default=0.4,
                   help="fraction of rows that are ambiguous/conflicting-signal (the messy middle)")
    ...
    if args.engine == "datadesigner":
        records = generate_datadesigner(args.n, args.fraud_rate, args.ambiguous_rate, args.seed, args.dd_model)
    else:
        records = generate_python(args.n, args.fraud_rate, args.ambiguous_rate, args.seed)
    _write(records)
