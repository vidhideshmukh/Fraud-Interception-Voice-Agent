...
# Mount the Barclays app demo page last, so it doesn't shadow the API
# routes above. Falls back silently if the directory doesn't exist yet
# (e.g. running just the API in a test environment).
try:
    app.mount("/app", StaticFiles(directory="demo/barclays_app", html=True), name="barclays_app")
except RuntimeError:
    pass
...
try:
    app.mount("/ops", StaticFiles(directory="demo/ops_dashboard", html=True), name="ops_dashboard")
except RuntimeError:
    pass
