...
def customer_says(session: CallSession, utterance: str) -> str:
    ...
    input_verdict = guardrails.check_input(
        text, run_self_check=(session.state != CallState.AWAITING_VERIFICATION and not _is_fast))
...
def _handle_verification_turn(session: CallSession, text: str) -> str:
    ...
    if anti_vishing.is_expired(session.session_id):
        ...
        return _say(session, "That code has expired for your safety. I have just sent a fresh "
                             "one to your Barclays app — please read the new code.")
    ...
    if anti_vishing.verify_code(session.session_id, text):
        ...
        reply = (f"Thank you, you're verified. The payment is £"
                 f"{txn.amount_gbp:,.0f} at {txn.merchant} in {txn.city}. "
                 f"Did you make this purchase?")
    ...
    return _say(session, "That code does not match what we sent to your app. Please open "
                         "the Barclays app notification and read the code exactly. "
                         "This code only verifies this call — we will never ask for your PIN, "
                         "full card number or password.")
...
def _freeze_channel(session: CallSession) -> str:
    ...
    return _say(session, "For your security, I'm ending this call and arranging a callback from "
                         "our fraud team, who will verify you through an alternate method.")
