    ...
    for txn in stream[start:end]:
        ...
        if flagged and txn.txn_id not in _triggered_txn_ids:
            ...
            # NOT re-expose it on later polls — see main.py's docstring.
                ...
                # If the orchestrator fails to start a session, we still want
                # to show the transaction in the stream, just without a
                # session_id or verification_code. Log the error but don't
                # crash the whole stream.
                logging.getLogger("transaction_stream").error(f"Error starting call for txn {txn.txn_id}: {e}")
