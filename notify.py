...
def _nemo_rails():
    ...
    if _rails is None:
        ...
        # NeMo Guardrails does NOT expand ${VAR} in config.yml, so point its model
        # at the SAME endpoint + model the rest of the app uses (from NIM_BASE_URL /
        # NIM_MODEL_GUARDRAILS) HERE. That makes switching to a local/cluster NIM
        # (e.g. http://gpu009:8000/v1) an .env change, not a config.yml edit.
        for m in config.models:
            if m.type == "main":
                m.model = GUARDRAILS_MODEL
                m.parameters = {**(m.parameters or {}), "base_url": NIM_BASE_URL}
